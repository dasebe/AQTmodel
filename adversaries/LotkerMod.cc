...
/**
 *  This a version of the Lotker adversaries adapted to a finite queue context
 * Generates and schedules injections for the nodes in the network.
 */
class LotkerMod : public AdvancedAdversary
{
protected:
...
Define_Module(LotkerMod);



void LotkerMod::injectInitialPackets()
{
...
void LotkerMod::injectPhasePackets()
{
    AdvSchedMess * tmp;
...
void LotkerMod::setLongPath(AdversarialInjectionMessage *message, int cur, bool lower)
{
...
void LotkerMod::singleEdgeConfinement(int targetGadget, double roundTime, SimTime baseTime)
{
    for (int i=1; i<=lengthn; i++)
    {
        AdvSchedMess * tmp;
...
void LotkerMod::cleanUpLatePackets(int targetChain)
{
